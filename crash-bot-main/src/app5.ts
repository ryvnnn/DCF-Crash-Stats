...
const setMultiplier = async (stopAt: number) => {
  try {
    ...
    //   )
    //   .then((result) => {
    const response = await axios.get(`${config.CRASH_API}/api/status/set_multiplier?multiplier=${stopAt}`, {
      headers: { Authorization: authKey.idToken },
    });
    console.log("API Response:", response.data);

    if (response.data.multiplier) {
          console.log(`Confirmed set multiplier to ${response.data.multiplier}`);
        }
...
const getAPIdata = async (stopAt: number) => {
  const response = await axios.get(`${config.CRASH_API}/api/status/set_multiplier?multiplier=${stopAt}`, {
    headers: { Authorization: authKey.idToken },
  });
  console.log("API Response:", response.data);
...
const parseNewGames = async (
  ...
  try {
    ...
    // console.log("new game meta data:", newGameMeta);
...
const getApiResult = async (roundNumber: number) => {
  const response = await axios.get(`${config.DEALER_API}roundId=${roundNumber}`, {
    headers: { Authorization: authKey.idToken },
  });
  console.log("API Response:", response.data);
};
...
const init = async () => {
  ...
  console.log('confirmed YEEEET 1');
  if (!(await signNonce())) return;
  getAPIdata(1.1);
  console.log('confirmed YEEEET 3');
  ...
  console.log('confirmed YEEEET 2');
  // getApiResult();
